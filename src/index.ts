...
const highlight_color_light = "background-color: rgba(230, 230, 140, 1)";
...
async function main() {
	...
	const renderCode = () => {
		...
		const result = jam.parseModule(updated_code);
		if (result == null) {
		...
		if (result.success) {
			root.innerHTML = "";
			parse_result = transformJsonAst(JSON.parse(result.ast));
			renderJson(parse_result, root, jsonViewStyles, onJsonItemHover);
			return;
		}

		const { error_line, error_column, error_message } = result;
		renderError(root, error_line, error_column, error_message);
	};
